...
for i in range(0,len(masks)):
    brain = nib.load(fname[i]).get_fdata().transpose((3,0,1,2))
    brains.append(brain)
...
model = ann_256_256_32(0.005, [256,256,32], decay_factor = 0.2).float()
MODEL_TYPE = "256_256_32"
CHECKPOINT = False

filepath = f'../mlp_models_3/full_brain_{MODEL_TYPE}'
...
if (CHECKPOINT):
    trainer = Trainer(max_epochs=300, gpus=[0], accumulate_grad_batches=2, logger=logger,
                      checkpoint_callback=checkpoint_callback,early_stop_callback=early_stop_callback, 
                      auto_lr_find=False, resume_from_checkpoint="../ann_gpu3_256_256_32/_ckpt_epoch_41.ckpt")
else:
    trainer = Trainer(max_epochs=300, gpus=[0], accumulate_grad_batches=2, logger=logger, 
                      checkpoint_callback=checkpoint_callback, early_stop_callback=early_stop_callback, 
                      auto_lr_find=False)
