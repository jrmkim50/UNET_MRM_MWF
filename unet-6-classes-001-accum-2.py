...
for number in numbers:
    ...
    label_names = ["w1", "w2", "w3", "t1", "t2", "t3"]
    subject_label_files = []
    for label_name in label_names:
        subject_label_files.append(file_root + label_name + ".nii.gz")
    label_files.append(subject_label_files)
    
trainLabels = np.array(label_files[:SPLIT])
trainBrains = np.array(fastt2_files[:SPLIT])
trainMaskArray = np.array(mask_files[:SPLIT])

validLabels = np.array(label_files[SPLIT:SPLIT+VAL])
validBrains = np.array(fastt2_files[SPLIT:SPLIT+VAL])
validMaskArray = np.array(mask_files[SPLIT:SPLIT+VAL])

stats = np.load("6_class_stats.npy")
    ...
    def __getitem__(self, idx):
        ...
        if (idx == self.end_brain_idx):
            ...
            while (patches_generating):
                ...
                label = np.zeros(brain.shape)
                label[0] = nib.load(self.label_files[self.brain_idx][0]).get_fdata() * mask
                label[1] = nib.load(self.label_files[self.brain_idx][1]).get_fdata() * mask
                label[2] = nib.load(self.label_files[self.brain_idx][2]).get_fdata() * mask
                label[3] = nib.load(self.label_files[self.brain_idx][3]).get_fdata() * mask
                label[4] = nib.load(self.label_files[self.brain_idx][4]).get_fdata() * mask
                label[5] = nib.load(self.label_files[self.brain_idx][5]).get_fdata() * mask
                for i in range(len(stats)):
                    label[i] -= stats[i][0]
                    label[i] /= stats[i][1]
                    label[i][mask == 0] = 0
        ...
        if self.transform is not None:
            ...
            signal_label = torch.tensor(signal_label)
            patches_mask = torch.tensor(patches_mask)
            if (idx != 2):
        ...
        return (torch.tensor(signal_graph).float(), torch.tensor(signal_label).float())
...
weight = 0
model = unet3d_tuning(0.001).float()
...
if (CHECKPOINT):
    trainer = Trainer(max_epochs=300, gpus=[2], logger=logger, accumulate_grad_batches=2, 
                      ...
                      auto_lr_find=False, 
                      resume_from_checkpoint="../unet_models/_ckpt_epoch_11.ckpt")
else:
    trainer = Trainer(max_epochs=300, gpus=[2], logger=logger, accumulate_grad_batches=2, 
                      checkpoint_callback=checkpoint_callback,
                      early_stop_callback=early_stop_callback, auto_lr_find=False) #2
